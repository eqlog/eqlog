...
const phl_theory cwf = {
    ...
    // surjective axioms:
    {
        ...
        subst_ty(id(G), A) -= A,
        subst_tm(id(G), a) -= a,
        subst_ty(g, subst_ty(f, A)) -= subst_ty(comp(g, f), A),
        subst_tm(g, subst_tm(f, a)) -= subst_tm(comp(g, f), a),
        dom(wkn(A)) -= ty_ctx(A),
        cod(wkn(A)) -= ctx_ext(A),
        tm_ty(var(A)) -= subst_ty(wkn(A), A),
        ...
        tm_ty(c) == Eq(a, b) |= a == b,
        Eq(a, b) == A && tm_ty(c) == A && tm_ty(d) == A |= c == d,
        tm_ty(refl(a)) -= Eq(a, a),
