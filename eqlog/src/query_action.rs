...
fn translate_premise(
    ...
    let premise = premise.iter().map(|atom| {
        use FlatAtom::*;
        match atom {
            Equal(_, _) => {
            ...
            Relation(rel, args) => {
                ...
                Query::Relation{relation: rel.clone(), projections, diagonals, results}
            },
            Unconstrained(tm, sort) => {
                fixed_terms.insert(*tm, sort.to_string());
                Query::Sort{sort: sort.clone(), result: *tm}
            },
        }
    }).collect();
    (premise, fixed_terms)
...
fn translate_conclusion(
    ...
    conclusion.iter().map(|atom| {
        ...
        match atom {
            ...
            Relation(rel, args) if args.is_empty() => {
                Action::AddTuple {
                    relation: rel.clone(),
                    args: Vec::new(),
                }
            ...
            Relation(rel, rel_args) => {
                ...
                if fixed_terms.contains_key(&result) {
                    ...
                    Action::AddTerm{function, args, result}
                }
            },
            Unconstrained(_, _) => {
