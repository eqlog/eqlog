    ...
    G: Fn(bool, bool) -> bool,
{
    fn emit_term_structure(&mut self, term: Term) {
        if self.structurally_added[term] { return; }
        ...
        match self.universe.data(term) {
            Variable(_) | Wildcard => {
                if self.unconstrained[term] {
                    self.flat_atoms.push(FlatAtom::Unconstrained(name, self.sorts[term].clone()));
                }
            },
            Application(func_name, args) => {
                let args: Vec<FlatTerm> =
                    args.iter().copied().map(|arg| self.flat_names[arg].unwrap())
                    .chain(once(name))
                    .collect();
                self.flat_atoms.push(FlatAtom::Relation(func_name.clone(), args));
            },
        };
    }
    ...
    fn emit_atom(&mut self, atom: &Atom) {
        ...
        match &atom.data {
            ...
            Predicate(pred, args) => {
                ...
                let args = args.iter().copied().map(|arg| self.flat_names[arg].unwrap()).collect();
...
pub fn flatten_sequent(sequent: &Sequent, sorts: &TermMap<String>) -> FlatSequent {
    ...
    let sequent = FlatSequent{premise, conclusion};
    #[cfg(debug_assertions)]
    sequent.check();
    sequent
}
